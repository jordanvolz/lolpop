...
4. Now we'll download the data for the example from Kaggle. If you already use kaggle from the command line you can simply execute the following: 
    ...
    unzip titanic.zip
    ```
    Or, Manually download the data from the following [link](https://www.kaggle.com/competitions/titanic/data) and unzip it. You should now have a `train.csv` and `test.csv` file in the `lolpop/examples/quickstart/titanic` directory. 
