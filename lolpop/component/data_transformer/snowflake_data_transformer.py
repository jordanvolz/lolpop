...
class SnowflakeDataTransformer(BaseDataTransformer): 
        ...
        if sql is None:
            if table is not None:            
                sql = "SELECT * FROM %s" %table
                 ...
                 raise Exception("Table or SQL command must be provided to SnowflakeDataTransformer in order to retrieve data.")
    ...
    def save_data(self, data, table, *args, **kwargs): 
        ...
        if tables.shape[0]> 0: 
           ...
           new_features = [x for x in data.columns if x.upper() not in table_schema.columns.str.upper()]
           deleted_features = [x for x in table_schema.columns if x.upper() not in data.columns.str.upper()]
           ...
           if len(new_features) > 0: 
                ...
                for col_name in new_features:
                    ...
                    sql = sql + ("ADD COLUMN %s %s" % (col_name.upper(), sf_type))
