...
class WandBHyperparameterTuner(AbstractHyperparameterTuner):
    ...
    def run_experiment(self, data, model_version, n_trials=100, timeout=600, *args, **kwargs):
        ...
        for k, v in data.items():
            vc_info = self.resource_version_control.version_data(
                model_version, v, key=k, file_suffix=k)
            self.metadata_tracker.register_vc_resource(
                model_version, vc_info, key=k, file_type="csv")

        #now, we determine overall best experiment and save into model_version
        winning_exp_id = self._get_winning_experiment(
            exp_list, perf_metric, reverse=utils.get_metric_direction(perf_metric))
        winning_exp = self.metadata_tracker.get_resource(
            winning_exp_id, parent=model_version, type="experiment")
        winning_exp_model_trainer = self.metadata_tracker.get_metadata(
            winning_exp, id="model_trainer")
        ...
        self.metadata_tracker.log_metadata(model_version, id="winning_experiment_id", data={
                                           "winning_experiment_id": winning_exp_id})
        self.metadata_tracker.log_metadata(model_version, id="winning_experiment_model_trainer", data={
                                           "winning_experiment_model_trainer": winning_exp_model_trainer})
    ...
    def _optuna_objective(self, trial, param_type, data, model_version, algo, params, metrics, perf_metric, experiment_list, model_list):
        ...
        #build model
        model, experiment = self.build_model(
            data, model_version, algo, model_params)
        #make predictions
    ...
    def _log_trial(self, trial, model_params, model, experiment, algo):
        #log params + trainer
        self.metadata_tracker.log_metadata(experiment, id="optuna_trial_number", data={
                                           "optuna_trial_number": trial.number})
        self.metadata_tracker.update_resource(
            experiment, {"start_time": trial.datetime_start, "end_time": datetime.utcnow()})
    ...
    def _log_study(self, study, model_version, algo):
        if optuna.visualization.is_available():
            ...
            self._save_plot("plot_optimization_history.html",
                            plot, model_version, algo)

            plot = optuna.visualization.plot_parallel_coordinate(study)
            self._save_plot("plot_parallel_coordinate.html",
                            plot, model_version, algo)

            plot = optuna.visualization.plot_param_importances(study)
            self._save_plot("plot_param_importances.html",
                            plot, model_version, algo)
    ...
    def _save_plot(self, name, plot, model_version, algo):
        ...
        artifact = self.metadata_tracker.log_artifact(
            model_version, id=plot_name, path=local_file, mime_type=plot_type, external=False)
