from component.model_visualizer.abstract_model_visualizer import AbstractModelVisualizer
...
class YellowbrickModelVisualizer(AbstractModelVisualizer): 
    ...
    def generate_viz(self, data, model, model_version): 
        ...
        if self.problem_type == "classification":
            ...
            self._save_plot(viz, data, model_version, "yb_classification_report")

            #confusion matrix
            viz = ConfusionMatrix(model)
            self._save_plot(viz, data, model_version, "yb_confusion_matrix")

            #rocauc
            viz = ROCAUC(model)
            self._save_plot(viz, data, model_version, "yb_rocauc")
            ...
            #self._save_plot(viz, data, model_version, "yb_prauc")

            #class error
            viz = ClassPredictionError(model)
            self._save_plot(viz, data, model_version, "yp_class_error")

            if classification_type == "binary":
                #discrimination threshold -- only works for binary
                viz = DiscriminationThreshold(model) 
                self._save_plot(viz, data, model_version, "yb_disctreshold")
    ...
    def _save_plot(self, viz, data, model_version, plot_name): 
